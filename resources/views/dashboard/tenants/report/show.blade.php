        ...
        <div class="col-lg-7">
            ...
            <div class="card content-card mb-4">
                <div class="card-body p-4">
                    <p class="text-muted mb-1">Reported on: {{ $report->reported_at ? $report->reported_at->format('d M Y, H:i') : 'N/A' }}</p>
            ...
            <div class="card content-card">
                 <div class="card-header bg-transparent p-4">
                    <h5 class="card-title mb-0">Admin Responses</h5>
                </div>
                    ...
                    @foreach($report->responses->sortByDesc('created_at') as $response)
                        <div class="d-flex gap-3 mt-4 {{ !$loop->last ? 'mb-4 pb-4 border-bottom' : '' }}">
                            ...
                            <div class="flex-grow-1">
                                ...
                                @endif
                                @if($response->notes)
                                <p class="mb-0"><small class="text-muted"><strong>Notes:</strong> {{ $response->notes }}</small></p>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="card-body p-4">
                 ...
                 @if($report->status === 'pending')
                    <div class="card-footer bg-transparent p-3 border-top">
                        <form action="{{ route('tenant.report.destroy', $report->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this report?');">
                            @csrf
                            @method('DELETE')
