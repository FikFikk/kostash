            ...
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    ...
                    @foreach ($rooms as $room)
                        <button type="button"
                            class="btn btn-sm btn-outline-primary room-filter-btn {{ request('room') == $room->id ? 'active' : '' }}"
                            data-room="{{ $room->id }}">
                            {{ $room->name }}
                        </button>
                    @endforeach
                            ...
                            @forelse($rooms as $room)
                                <tr class="room-row" data-room="{{ $room->id }}">
                                    ...
                                    @for ($month = 1; $month <= 12; $month++)
                                        @php
                                            $reading = ($roomMeters[$room->id] ?? collect())->firstWhere(
                                                'month',
                                                $month,
                                            );
                                        @endphp
                                ...
                                @if (count($roomMeters[$room->id]) > 6)
                                    <div class="text-center mt-2">
                                        <button type="button" class="btn btn-sm btn-outline-primary"
                                            onclick="showAllReadings('{{ $room->id }}')">
    ...
    @push('scripts')
        ...
        `;
            ...
            window.showRoomDetails = window.showAllReadings = async function(roomId) {
                ...
                const roomName = roomRow ? roomRow.querySelector('.fw-medium')?.textContent?.trim().replace(/^\S+\s*/,
                    '') : `Room ${roomId}`;
                MeterApp.showLoadingInModal(`${roomName} - ${currentYear} Readings`);
                let url = '{{ route('dashboard.meter.room.details', ':roomId') }}'.replace(':roomId',
                    encodeURIComponent(roomId)) + `?year=${currentYear}`;
